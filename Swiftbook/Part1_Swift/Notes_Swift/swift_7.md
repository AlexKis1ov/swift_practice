...
- Структуры НЕ являются ссылочным типом (reference type), а являются типом значения (value type). 2 экземпляра структуры не могут ссылаться на одну и ту же область памяти
...
❗️ Если бы свойство rectangle было бы коснтантой, то вызов метода scale вызвал бы ошибку. При изменении свойств стурктуры свойство экземпляра должно быть переменной (var). Само свойство внутри структуры так же должно быть в виде переменной
